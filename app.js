...
const CAT_PALETTES = [
  ...
  { color: '#9400D3', bg: '#F3E8FD', icon: '🔮' },
  { color: '#D85A30', bg: '#FAECE7', icon: '🧡' },
];
    ...
    html += `
      <div class="cat-section ${isCollapsed ? "collapsed" : ""}">
        <div class="cat-section-header" onclick="toggleSection('${cat.id}')">
          <div class="cat-icon-wrap" style="background:${palette.bg}"><span>${palette.icon}</span></div>
          ...
          <button class="cat-section-del" data-id="${cat.id}" data-name="${safeName}"
            onclick="event.stopPropagation();deleteCat(this.dataset.id,this.dataset.name)" title="Eliminar">×</button>
    ...
    html += `
      <div class="cat-section ${isCollapsed ? "collapsed" : ""}">
        <div class="cat-section-header" onclick="toggleSection('__uncategorized__')">
          <div class="cat-icon-wrap" style="background:var(--gray-100)"><span>📦</span></div>
