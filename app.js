...
function renderIconPicker() {
  const el = $("icon-picker"); if (!el) return;
  el.innerHTML = CAT_ICONS.map(ic => `
...
function renderColorPicker() {
  const el = $("color-picker"); if (!el) return;
  el.innerHTML = CAT_COLORS.map((c, i) => `
...
function productCardHTML(p, isSingle, idx) {
  ...
  if (p.price) {
    ...
    const pricePerUnit = packSize > 1 ? (p.price / packSize) : p.price;
    const priceLabel = packSize > 1
      ? `${pricePerUnit.toFixed(2)} €/ud <span style="opacity:0.6;font-size:10px">(pack ${p.price.toFixed(2)}€)</span>`
      : `${p.price.toFixed(2)} €/ud`;
    tags.push(`<span class="tag">💰 ${priceLabel}</span>`);
  }
...
async function init() {
  try {
    const [limSnap, cartSnap] = await Promise.race([
      Promise.all([configRef.child("limits").get(), cartRef.get()]),
    ...
    initListeners(); // intenta conectar igualmente
  }
  hideSplash();
}
