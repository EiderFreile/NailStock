...
// ---- Firebase refs ----
// Prefijo nailstock/ para no mezclar con otras apps del mismo proyecto Firebase
...
function renderCats() {
  ...
  el.innerHTML = state.categories.map(c => {
    // Escapamos el nombre para usarlo en data-attribute, evitando problemas con comillas
    const safeName = c.name.replace(/&/g,'&amp;').replace(/"/g,'&quot;');
    return `
      <div class="cat-pill ${state.selectedCat === c.id ? "active" : ""}"
           data-id="${c.id}" onclick="selectCat('${c.id}')">
        ${c.name}
        <button class="cat-pill-del"
                data-id="${c.id}"
                data-name="${safeName}"
...
function openProductModal(productId) {
  // Si recibimos un ID buscamos en state, si recibimos null es producto nuevo
...
function productCardHTML(p, isSingle, idx) {
  ...
  // Usamos data-id para evitar problemas con comillas en nombres de producto
  return `
    <div class="product-card ${statusClass}"
         data-id="${p.id}"
         onclick="openProductModal(this.dataset.id)"
